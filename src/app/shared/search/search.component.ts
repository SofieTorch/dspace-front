...
export class SearchComponent implements OnInit, OnDestroy {
   ...
   */
  chartFiltersRD$: BehaviorSubject<RemoteData<SearchFilterConfig[]>> =
      ...
      combineLatest([
        ...
        .subscribe(
          ([configuration, searchSortOptions, searchOptions, sortOption]: [
            ...
            const searchOptionsConfiguration =
              searchOptions.configuration || configuration;
            const combinedOptions = Object.assign({}, searchOptions, {
              configuration: searchOptionsConfiguration,
              sort: sortOption || searchOptions.sort,
              forcedEmbeddedKeys: [
                this.forcedEmbeddedKeys.get(searchOptionsConfiguration),
                'thumbnail',
              ],
            });
  ...
  private retrieveFilters(searchOptions: PaginatedSearchOptions) {
    ...
    this.searchConfigService
      ...
      .subscribe((filtersRD: RemoteData<SearchFilterConfig[]>) => {
        const filtersPayload = filtersRD.payload.filter(
          (entry: SearchFilterConfig) => !this.chartReg.test(entry.filterType)
        );
        const chartFiltersPayload = filtersRD.payload.filter(
          (entry: SearchFilterConfig) => this.chartReg.test(entry.filterType)
        );
